...
import csv
# xml_to_anno_lbls(meta_data, xml_data, src_full_file_path, op_year)
...
def annos_to_csv(in_full_directory_path, s3_bucket_source_name):
    ...
    for counter,anno_full_file_name in enumerate(annos):
        ...
        with open(anno_full_file_name, 'r') as f:
            ...
            anno, lbls = helpers.xml_to_anno_lbls(meta_data, xml_data, adjsuted_new_file_name, op_year)

            # print("anno:", anno)
            # print("lbls:", lbls)

            helpers.anno_lbl_to_csv(ALL_ANNOS, anno, lbls, ALL_LBLS, append=True, append_count=counter)
